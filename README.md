...
> **事务化更新**：脚本先把所有分类（Bangumi + 豆瓣 + 手动条目）同步到内存，封面先下载到
> `images/.staging/` 暂存区。**全部分类都成功**才会清空旧封面、移入新封面并写入
> `data/*.json`；任何一个分类失败（API 报错、断网等）会立即回滚，仓库数据保持原样，
> 不会产生提交。GitHub Actions 每次同步失败时会显示红色 ❌，但不会破坏线上数据。
